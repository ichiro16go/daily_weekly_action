...
from jira_monitor import JiraClient
...
def main() -> None:
    ...
    key = args.ticket.upper()
    if not key.startswith("JPREQ-"):
        print(f"❌ エラー: 対象は JPREQ-* チケットのみです（指定: {key}）", file=sys.stderr)
        sys.exit(1)
    ...
    # Closeチケットは評価するがコメント投稿しない
    is_closed = status_name in CLOSE_STATUSES

    result = analyze(issue)
    cli_output = format_cli(key, title, result)
    print(cli_output)
    ...
    if is_closed:
        print(f"⚠️ ステータスが '{status_name}' のためコメントは投稿しません。")
        return

    # 重複チェック
    try:
        if has_recent_marker(client, key):
            print(f"⚠️ 直近のコメントに既に {MARKER} があります。スキップします。")
            return
    except Exception:
        pass  # 重複チェック失敗は無視して続行

    comment_text = format_jira_comment(key, title, result)
    try:
        post_comment(client, key, comment_text)
        print(f"✅ {key} にコメントを投稿しました。")
    except Exception as e:
        print(f"❌ コメント投稿エラー: {e}", file=sys.stderr)
        sys.exit(1)
