...
| `/calendar` | メンバーごとの未完了チケット週次・月次ガント風ビュー |
| `/closed` | 週次クローズ一覧（担当・タイプ・LT） |
| `/health` | チーム健全性（滞留・期限超過・放置チケット＋期間ラベル別継続状況） |

> `/members` は廃止（個人完了数の信頼性問題のため → `assignee` が対応・確認・クローズで変わる）。  
> `/ranking` `/cohort` は `/health` に統合。旧URLは `/health` にリダイレクト。
...
> メンバー別の完了数・クローズ一覧は、クローズ後の自動再担当を避けるため changelog からクローズ時点の担当を復元して集計します。
> `/closed` には `assignee_source` と週ごとの `assignee_source_counts` を出して、changelog 由来か current fallback かを確認できます。

## セットアップ

### 前提条件

- Node.js 20.x（Vercel の実行環境も `package.json` の `engines.node` で 20.x に固定）
- Python 3.12+
